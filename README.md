...
To obtain a backdoor detector G for badnets, we first need to "repair" the badnet B itself. In this work, we "repaired" the badnet by employing STRong Intentional Perturbation (STRIP), fine-pruning and anti-repairedNet defenses. While the both STRIP and fine-pruning defenses are based on existing works in the literaure, we proposed a modification to the fine-pruning defense approach. More importantly, the anti-repairedNet defense is a new defense that is (to the best of author's knowledge) proposed for the first time in this work. Rather than conventional approach which repairs the badnet in order to decrease attack success rate (while maintaining high clean accuracy), the proposed anti-repairedNet defense further destroys the badnet so that high attack success rate is achieved (while maintaining very low clean accuracy).
...
Appraoch employ for the fine-pruning defense is presented as thus.
