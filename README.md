...
To obtain the a backdoor detector G for badnets first we need to "repair" the badnet B itself. In this work, we repaired the badnet by employing STRong Intentional Perturbation (STRIP) and fine-pruning defenses. While the STRIP defense is based on what existing works in the literaure, we have proposed a modification to the fine-pruning defense approach.  More importantly, we propose a new approach which further "destroys" the badnet rather than repair it. We call this defense anti-repairedNet defense. The idea behind this approach is to 
After considering The STRong Intentional Perturbation (STRIP) defense against the trojan trigger-based backdoor attacks, and fine-pruning, we decided to adapt the later approach due to the good performance it demonstrates against the pruning-aware attacks (for more details check the [project report](https://github.com/vbabushkin/ECE_GY_9163_Machine_Learning_for_Cyber_Security_Project/blob/main/ECE_GY_9163_PROJECT_REPORT.pdf
