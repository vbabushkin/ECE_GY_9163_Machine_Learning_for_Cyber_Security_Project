...
code perturbations to the clean input image is less visible in some channels of the conv_2 layers
the activation map of the different layers of the badNet were obtained and analyzed. From the analysis of the activation maps, we observed the features of the input  
...
While the STRIP approach still allows to achieve clean classification accuracy around 88% for sunglasses badnet model, its attack success rate is higher than that of fine-pruning (9.78% vs 0.69%), which affected our decision to dismiss this defense mechanism in favour of fine-pruning. 
